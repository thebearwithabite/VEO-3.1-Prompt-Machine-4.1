...
## 2024-05-18 - [ConfirmDialog Reusability & A11y]
**Learning:** Reusable confirmation dialogs often lack contextual clarity if their action button text is hardcoded. Furthermore, screen readers need `role="dialog"` and appropriate aria labels to parse modal context properly. Strict DOM querying in tests (like Playwright) becomes vital when multiple instances of generic components like ConfirmDialog exist in the DOM (e.g., hidden vs visible dialogs).
**Action:** When creating or modifying generic modal components, expose props for the action button text (`confirmText`, `cancelText`) and semantic styling (`isDestructive`) so context is maintained across usages. Ensure proper ARIA attributes (`aria-modal`, `aria-labelledby`, `role="dialog"`) and test using highly specific locators.
